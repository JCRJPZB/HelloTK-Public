    ...
    execute: function () {
      ...
      _export("Formation", Formation = (_dec = ccclass('Formation'), _dec(_class = (_temp = class Formation extends (_crd && UIBase === void 0 ? (_reportPossibleCrUseOfUIBase({
        ...
        onInit() {
          ...
          for (let i = 0; i < this.max_selection; i++) {
            // 生成武将头像
        ...
        setFormationBar() {
          let conf = (_crd && Configure === void 0 ? (_reportPossibleCrUseOfConfigure({
            error: Error()
          ...
          for (let i = 0; i < conf.length; i++) {
            ...
            let ctrl = node === null || node === void 0 ? void 0 : node.getComponent(_crd && FormationBar === void 0 ? (_reportPossibleCrUseOfFormationBar({
              error: Error()
          ...
          for (let i = 0; i < this.ids.length; i++) {
            ...
            if (hero_portrait) {
              ...
              (_hero_portrait$getCom = hero_portrait.getComponent(_crd && HeroPortrait === void 0 ? (_reportPossibleCrUseOfHeroPortrait({
                error: Error()
              }), HeroPortrait) : HeroPortrait)) === null || _hero_portrait$getCom === void 0 ? void 0 : _hero_portrait$getCom.init(this.heros_conf_map.get(this.ids[i])); // 初始化
        ...
        changeSelected(id, is_select, call_back) {
          if (!is_select) {
            ...
            if (this.selected_hero_ids.length >= this.max_selection) {
              ...
              // 如果选中的武将数量大于等于最大编入人数
              let canvas_pos = (_find = find("Canvas")) === null || _find === void 0 ? void 0 : _find.worldPosition; // 计算提示Label显示的位置

              let portrait_pos = (_this$hero_map$get = this.hero_map.get(id)) === null || _this$hero_map$get === void 0 ? void 0 : _this$hero_map$get.worldPosition;
        ...
        updateFormation(update_hero, udpate_pos) {
          ...
          if (update_hero) {
            ...
            for (let i = 0; i < this.portraits.length; i++) {
              ...
              let hero_id = this.heros_conf_map.get(this.selected_hero_ids[i])["hero_id"]; // 注意一件事，id和hero_id是不一样的，一个是当前这个武将的id，一个是武将配置中的武将id，即一个是每个玩家不一样一个是所有该武将都一样
