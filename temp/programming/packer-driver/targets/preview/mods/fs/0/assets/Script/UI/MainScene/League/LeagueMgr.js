    ...
    execute: function () {
      ...
      _export("LeagueMgr", LeagueMgr = (_dec = ccclass('LeagueMgr'), _dec(_class = (_temp = _class2 = class LeagueMgr extends Component {
        ...
        constructor(name) {
          ...
          var dynamics = this.loadDynamics();
          dynamics.forEach(dynamic => {
            this.dynamic_map.set(dynamic["league_id"], dynamic);
          });
          var shop_list = this.loadShopList();
          ...
          (_crd && Subscribe === void 0 ? (_reportPossibleCrUseOfSubscribe({
            ...
            //      更新消息至服务器 这里先手动改   #
            // ####################################
            var info = this.info_map.get(league_id); // Map中存放的是引用不是深拷贝的对象，所以修改Map中的元素，原数组中的元素也会对应修改
          ...
          (_crd && Subscribe === void 0 ? (_reportPossibleCrUseOfSubscribe({
            ...
            //      更新消息至服务器 这里先手动改   #
            // ####################################
            var info = this.info_map.get(league_id);
            ...
            info["num_of_member"]--;
            var member_list = info["member_list"];
            var idx = member_list.indexOf(pid);
