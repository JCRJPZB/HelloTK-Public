      ...
      _export("Mail", Mail = (_dec = ccclass('Mail'), _dec(_class = (_temp = class Mail extends (_crd && UIBase === void 0 ? (_reportPossibleCrUseOfUIBase({
        ...
        refreshMail() {
          ...
          if (this.mail_title_root) {
            (function () {
              var list = (_crd && MailMgr === void 0 ? (_reportPossibleCrUseOfMailMgr({
                error: Error()
              }), MailMgr) : MailMgr).getInstance().getMailList();

              if (list.length > _this.mail_list.length) {
                var _loop = function _loop(i) {
                  _this.mail_list.push(list[i]["mail_id"]);

                  _this.mail_map.set(list[i]["mail_id"], list[i]);

                  var new_title = _this.gen.generator(_this.mail_title_root, "Mail_Title");

                  var ctrl = new_title === null || new_title === void 0 ? void 0 : new_title.getComponent(_crd && MailTitle === void 0 ? (_reportPossibleCrUseOfMailTitle({
                    error: Error()
                  }), MailTitle) : MailTitle);

                  if (ctrl) {
                    ctrl.init(list[i], () => {
                      _this.readMail(list[i]["mail_id"]);
                    });
                  }
                };

                for (var i = _this.mail_list.length; i < list.length; i++) {
                  _loop(i);
                }
              }

              _this.readMail(_this.mail_list[0]);
            })();
          }
        }
        ...
        readMail(mail_id) {
          if (this.mail_sender && this.text_content) {
            var mail = this.mail_map.get(mail_id);
