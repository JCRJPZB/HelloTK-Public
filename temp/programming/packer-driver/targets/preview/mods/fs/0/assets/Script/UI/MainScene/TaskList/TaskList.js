    ...
    execute: function () {
      ...
      _export("TaskList", TaskList = (_dec = ccclass('TaskList'), _dec(_class = (_temp = class TaskList extends (_crd && UIBase === void 0 ? (_reportPossibleCrUseOfUIBase({
        ...
        showTasksByCategory(cate) {
          ...
          this.curr_cate = cate;
          var task_map = (_crd && TaskMgr === void 0 ? (_reportPossibleCrUseOfTaskMgr({
            error: Error()
        ...
        genTask(conf, map) {
          if (conf["pre_task"]) {
            var pre_task = map.get(conf["pre_task"]);
          ...
          if (this.task_root) {
            var task_node = (_crd && Generator === void 0 ? (_reportPossibleCrUseOfGenerator({
              error: Error()
            }), Generator) : Generator).getInstance().generator(this.task_root, "Task");
            var ctrl = task_node === null || task_node === void 0 ? void 0 : task_node.getComponent(_crd && Task === void 0 ? (_reportPossibleCrUseOfTask({
              error: Error()
            }), Task) : Task);
