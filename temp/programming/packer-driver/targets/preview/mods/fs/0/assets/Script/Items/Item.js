    ...
    execute: function () {
      ...
      _export("Item", Item = (_dec = ccclass('Item'), _dec(_class = (_temp = class Item extends Component {
        ...
        init(conf, item_num, browser_root) {
          ...
          var bg_spf = this.imgMgr.getImg("Item_Bg_" + this.data["rare"]);
          var sp_spf = this.imgMgr.getImg(this.data["img"]);
        ...
        createBrowser(parent) {
          ...
          var browser = this.gen.generator(parent, "Item_Browser");

          if (!browser) {
            return;
          }

          var ctrl = browser.getComponent(_crd && ItemBrowser === void 0 ? (_reportPossibleCrUseOfItemBrowser({
            error: Error()
